/* ESLint — le filet GÉNÉRIQUE ; celui du dépôt est `outils/gardien.js`, dont
   aucune des six pannes n'est visible d'ici. Ce qu'ESLint attrape seul :
   identifiant fautif, variable morte, clé dupliquée.
   TROIS BLOCS pour trois natures de fichiers, une seule difficulté : les pages
   ne chargent AUCUN module ES — un `<script src>` partage la portée globale
   (§9), donc `frise.js` emploie `escapeH` sans l'avoir déclaré.
   LA LISTE DES GLOBALS VIENT DU GARDIEN (R2), jamais d'ici : la recopier
   l'aurait fait vieillir au premier module ajouté (§12). */
const js      = require("@eslint/js");
const globals = require("globals");
const { PAGES, nomsExposes, declarationsDeHautNiveau } = require("./outils/gardien.js");

/* Ce que ses VOISINS posent dans la portée globale d'un fichier de page. En
   `writable` : `CONTENU` est déclaré par `noyau.js` et réaffecté par
   `contenu-io.js`, vie normale d'un global de script. */
...
const REGLES = {
  ...
  /* Deux assouplissements, pas un de plus : un `localStorage` refusé ne doit pas
     casser la partie (`catch(e){}`), et `vars:"local"` parce que les noms de
     haut niveau d'une page SONT sa surface publique — R5 sait lire un
     `onclick=`, pas ESLint. */
