...
>
> **État au 14 août 2026.** Décrit le code tel qu'il tourne. Deux parties : **I. Le sens** (ce que le jeu veut dire) puis **II. Le système** (comment il est fait). Ce qui est décidé et pas encore codé se dit en toutes lettres, là où ça se décide — les **points ouverts** du §7 pour le sens, les lignes du §15 pour le système. **Le corps du texte décrit l'état actuel, pas la façon dont on y est arrivé** — c'est pourquoi deux choses n'y sont pas : l'historique des révisions vit dans `docs/HISTORIQUE.md`, le récit des pannes déjà réparées au §2 de `docs/PASSATION.md`. La **discipline d'écriture**, qui ne touche pas au moteur, vit dans `docs/ECRITURE.md`.
...
**`outils/gardien.js`** (`npm run gardien`, dans `npm test` après les suites) rend ces conventions opposables. **Dix règles, dix pannes réellement vécues**, chacune citant le § qui la tranche. *Ce qu'elles surveillent une par une est dans `docs/CARTE.md` — et dans l'en-tête du fichier lui-même, qui est la seule liste que le code puisse contredire.* Il ne connaît **ni pièce, ni empan, ni valeur** — même discipline que les suites. Ce qui se décide ici, c'est ce qu'une règle du gardien **a le droit d'être** : le constat d'une panne payée, pas une préférence de style ; et un motif vérifiable sur le source, pas une intention.
