...
>
> **État au 14 août 2026.** Décrit le code tel qu'il tourne. Deux parties : **I. Le sens** (ce que le jeu veut dire) puis **II. Le système** (comment il est fait). Ce qui est décidé et pas encore codé se dit en toutes lettres, là où ça se décide — les **points ouverts** du §7 pour le sens, les lignes du §15 pour le système. L'historique des révisions vit en fin de document (§18), et le récit des pannes déjà réparées dans le §2 de `docs/PASSATION.md` : **le corps du texte décrit l'état actuel, pas la façon dont on y est arrivé.**
...
**`outils/gardien.js`** (`npm run gardien`, dans `npm test` après les suites) rend ces conventions opposables. Dix règles, dix pannes réellement vécues, chacune citant le § qui la tranche : R1 la forme des balises que le harnais inline ; R2 les collisions de noms de haut niveau, **par page** ; R3 les variables CSS employées sans définition ; R4 les familles CSS sans porteur ; R5 les `onclick=` qui visent une fonction inexistante ; R6 les ids visés, et nommément les quatre ancres du tutoriel ; R7 les restes du schéma 2 ; R8 les tailles annoncées par `docs/CARTE.md` ; R9 les lectures d'`attend`/`apres` posés sur une remise ; R10 la recopie à la main d'un prédicat que `regles.js` exporte. Il ne connaît **ni pièce, ni empan, ni valeur** — même discipline que les suites.

**R7, R9 et R10 marchent sur `app/`, `tests/` et `outils/`** ; les sept autres sur les deux pages. Les suites et les outils sont couverts pour la même raison que le jeu : ce sont eux qui *reflètent* les règles, et un reflet qui ne rappelle pas ce qu'il reflète est précisément le défaut que ce fichier passe son temps à décrire. Avant d'ajouter une règle, la question à poser est donc **sur quel territoire elle marche** — la réponse n'est pas « `app/` » par défaut.
