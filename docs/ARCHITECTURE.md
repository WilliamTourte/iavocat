...
*L'accusé migrait sur la **remise** — la forme d'avant le §3 — jusqu'au 14 août, et c'était le dernier endroit du dépôt qui **produisait** l'ancienne écriture.* Le prix était silencieux : `attentesDe` ne rend une paire que si `attend` existe **sur la remise**, si bien qu'un `apres` posé seul n'était lisible ni par le jeu ni par l'atelier — `attentesDeRemise` rendant `[]` pareillement, l'inspecteur ne pouvait même pas l'éditer. Une donnée migrée puis injoignable est pire qu'une donnée perdue : elle se voit dans le JSON et ne se voit nulle part ailleurs. **On n'invente aucune attente pour autant** : si la session n'en déclare aucune, l'accusé reste où il est. Une attente sans `attend` serait toujours trouvée non servie par `attenteCourante` — elle deviendrait l'attente courante pour toujours, et bloquerait l'avancement de session. `migrerContenu` reste tenue par R9 en **lecture** (c'est un convertisseur, il doit relire l'ancienne forme) ; ce qui a changé, c'est qu'il ne l'**écrit** plus. **Le jeu, lui, ne migre pas** : un contenu de schéma 2 est refusé par `contenuValide()`, et le jeu **ne joue rien** — il affiche un bandeau qui nomme le cas (fichier absent, schéma trop ancien, clé manquante), repasser par l'atelier.
...
**Le contrat va plus loin qu'on ne le tenait : une suite ne REDÉCIDE rien non plus.** Le harnais `require` les modules du jeu — `moteur.js` pour les projections depuis le 3 août, et `regles.js` **depuis le 14** — au lieu de réécrire ce qu'ils publient. Trois recopies vivaient là, et aucune suite ne pouvait les voir puisque les suites ne se lisent pas elles-mêmes : le prédicat `estRegle`, écrit à la main quatre fois ; les prédicats du vice, dédoublés entre les sélecteurs de fenêtre et ceux de `surContenu` ; et l'index du terme qui prend un empan, écrit cinq fois en `findIndex` alors que `regles.js` l'exporte sous le nom `indexTermeChamp`. Le principe est celui du §12, appliqué au dernier territoire : **une suite désigne, elle ne décide pas.** C'est R10 qui le tient désormais.
...
**`outils/gardien.js`** (`npm run gardien`, dans `npm test` après les suites) la rend opposable. Dix règles, dix pannes réellement vécues, chacune citant le § qui la tranche : R1 la forme des balises que le harnais inline ; R2 les collisions de noms de haut niveau, **par page** ; R3 les variables CSS employées sans définition ; R4 les familles CSS sans porteur ; R5 les `onclick=` qui visent une fonction inexistante ; R6 les ids visés, et nommément les quatre ancres du tutoriel ; R7 les restes du schéma 2 ; R8 les tailles annoncées par `docs/CARTE.md` ; R9 les lectures d'`attend`/`apres` posés sur une remise ; R10 la recopie à la main d'un prédicat que `regles.js` exporte. Il ne connaît **ni pièce, ni empan, ni valeur** — même discipline que les suites.

**Il marche sur `app/`, `tests/` et `outils/`.** Ce n'était pas le cas : jusqu'au 14 août, R7 et R9 portaient `marcher("app")` en dur, et les huit autres ne regardaient que les deux pages. Les suites et les outils — **2 800 lignes** — n'étaient donc tenus que par ESLint, alors que ce sont eux qui *reflètent* les règles, et qu'un reflet qui ne rappelle pas ce qu'il reflète est précisément le défaut que ce fichier passe son temps à décrire. Le relevé d'extension a trouvé le territoire **sain sur R7 et R9**, et fautif sur R10 : `tests/` recopiait le prédicat `estRegle` **quatre fois à la main**. On étend un filet sur un territoire sain — c'est le bon moment, pas le mauvais.

**R10 est la sœur de R2, un cran plus haut.** R2 interdit à deux fichiers d'une même page de se disputer un *nom* ; R10 interdit à n'importe quel fichier de réécrire une *décision* que `regles.js` publie déjà. Le prédicat `estRegle` vit hors de la fabrique **exprès**, pour qu'on puisse le poser sans `JEU` lié (§12) : `app/atelier/noyau.js` l'appelle, le harnais ne l'appelait pas — il `require` pourtant `moteur.js` depuis toujours, et pour cette raison exacte. La règle se dit en une phrase : *un fichier qui veut savoir si une pièce est une règle le demande, il ne le redécide pas.* Sa forme vérifiable est le motif `includes("règle")` hors d'`app/regles.js`.

**R9 est la sœur de R7, et pour la même raison.** R7 interdit de déplier un lien du schéma 2 (`l.a[0]`), parce qu'une branche y était restée sans qu'aucune suite ne bronche. R9 interdit de **lire** `attend` ou `apres` sur une remise, parce qu'une branche du diagnostic y était restée depuis que la remise attend une *liste* (§3) : six fausses informations à chaque ouverture de l'atelier. **Quatre fonctions** sont nommées en exception, et seulement elles — les deux normalisateurs déclarés au §11, `attentesDe` (`app/regles.js`) et `attentesDeRemise` (`app/atelier/noyau.js`), *on ne les fusionne pas, on dit lequel est lequel* ; et les deux convertisseurs, `attentesEditables` (`frise.js`), qui convertit une remise en place à la première édition, et `migrerContenu` (`contenu-io.js`), qui relit un contenu de schéma 2. Les **écritures** restent libres : construire ou défaire l'ancienne forme est permis, c'est la *lire* au lieu de passer par un normalisateur qui ne l'est plus. Une forme d'écriture qu'on continue de lire doit se déplier en un seul endroit.
...
*En l'écrivant, un défaut du gardien lui-même : les lignes qu'il annonce étaient fausses.* `decouperJS` blanchissait les commentaires de bloc et le texte des gabarits **en les raccourcissant**, si bien que R7 et R9 comptaient trop peu de sauts de ligne — la première mesure envoyait à la ligne 123 pour un écart qui vit à la 153, et à la 373 pour la 378. Un message qui désigne la mauvaise ligne coûte plus qu'il ne rapporte. Les sauts de ligne sont désormais restitués ; rien d'autre ne change, un `\n` étant du blanc pour toutes les autres règles.
