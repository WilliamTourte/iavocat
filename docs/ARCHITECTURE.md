...
**`outils/gardien.js`** (`npm run gardien`, dans `npm test` après les suites) la rend opposable. Huit règles, huit pannes réellement vécues, chacune citant le § qui la tranche : R1 la forme des balises que le harnais inline ; R2 les collisions de noms de haut niveau, **par page** ; R3 les variables CSS employées sans définition ; R4 les familles CSS sans porteur ; R5 les `onclick=` qui visent une fonction inexistante ; R6 les ids visés, et nommément les quatre ancres du tutoriel ; R7 les restes du schéma 2 ; R8 les tailles annoncées par `docs/CARTE.md`. Il ne connaît **ni pièce, ni empan, ni valeur** — même discipline que les suites.
